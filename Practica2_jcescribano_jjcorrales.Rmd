...
* Correlación entre age, chol, trtbps, thalachh

* Sex vs target
  + Determinar si hay una diferencia en la distribución de hombre y mujeres entre las categorias 0 y 1 de target.
  + Usaremos el test chi-cuadrado

* Age vs target
  + Relación entre edad y probabilidad de ataque cardiaco
  ...
  + Usaremos un gráfico de dispersión, no tengo muy claro como hacerlo

* Regresión logistica target -> chol, Age, sex, trtbps
  + Se particionaran los datos para disponer de un conjunto de entrenamiento y test
  + Se evaluará la calidad del modelo
...
Establecemos que el p-valor de significancia será de 0.05

```{r normalidad}
qqnorm(data_heart$age)
qqline(data_heart$age, col=2)
shapiro.test(data_heart$age)
...
qqnorm(data_heart$thalachh)
qqline(data_heart$thalachh, col=2)
shapiro.test(data_heart$thalachh)
```

Los resultados indican que todas las variables numéricas del dataset menos age no son normales ya que su p-value es menor al p-value de significancia (0.05)
...
```{r homocholoutput}
leveneTest(thalachh ~ output, data=data_heart)
```
No existe homogeneidad de la varianza entre chol y output (p-value<0.05)
...
```{r correlacion}
cor(data_heart[,c("age","chol", "trtbps", "thalachh")])
```
No se observa una correlación fuerte entre las variables estudiadas aunque podrían ser relevantes las correlaciones entre age y thalachh y age y trtbps
...
```
Como resultado del test chi-cuadrado obtenemos un p-value<0.05 lo que significa que existe una diferencia significativa en la distribución de hombres y mujeres con respecto a la probabilidad de tener un ataque cardíaco.
...
t.test(mas_probabilidad$age, menos_probabilidad$age)
```
Para realizar este test los datos deben ser normales y con varianzas similares. La variable age sigue una distribución normal según los tests realizados pero no tengo claro si las varianzas lo son.
